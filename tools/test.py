...
def scan_code(session2):
    ...
    img = qr.make_image()
    pil_image_change = img.resize(
        (200, 200), resample=Image.BICUBIC, box=None, reducing_gap=None
    )
...
def verification():
    ...
    if resp1["data"]["isLogin"]:
        ...
        pil_image_change = pil_image.resize(
            (200, 200), resample=Image.BICUBIC, box=None, reducing_gap=None
        )
        tk_image = ImageTk.PhotoImage(pil_image_change)
...
def cancel_login():
    msg1 = messagebox.askyesno(
        title="提示", message="注销后cookie将失效，是否注销登录？"
    )
    if msg1:
